...
export async function GET(
  request: NextRequest,
  ctx: { params: Promise<{ projectId: string; specId: string; action: string }> }
) {
  const identity = await getCurrentProjectIdentity(request)
  if (!identity.userId) return Response.json({ error: "Unauthorized" }, { status: 401 })

  const { projectId, specId, action } = await ctx.params
  if (action !== "download") return specNotFoundResponse("Not found")
  ...
  return specMarkdownResponse(spec.content, {
    downloadFilename: `spec-${specId}.md`,
  })
}
